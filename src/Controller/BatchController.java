...
public class BatchController {
    ...
    public static String getBatchID(String year,int number) throws ClassNotFoundException, SQLException{
        String sql = "Select batchID From Batch where year='" + year + "'and number = '"+number+"'";
        ResultSet rst = DBHandler.getData(DBConnection.getDBConnection().getConnection(), sql);         
        while(rst.next()){                 
            return rst.getString(1);
    ...
    public static String getAcademeciYear(String batchID) throws ClassNotFoundException, SQLException{
         String sql = "Select year From Batch where batchID='" + batchID +"'";
        ResultSet rst = DBHandler.getData(DBConnection.getDBConnection().getConnection(), sql);         
        while(rst.next()){                 
            return rst.getString(1);
        }             
    ...
    public static String getBatchNumber(String batchID) throws ClassNotFoundException, SQLException{
         String sql = "Select number From Batch where batchID='" + batchID +"'";
        ResultSet rst = DBHandler.getData(DBConnection.getDBConnection().getConnection(), sql);         
        while(rst.next()){                 
            return rst.getString(1);
        }             
